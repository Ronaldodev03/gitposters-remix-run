...
export async function getAllPostsWithDetails({
  ...
  // seleccionamos la info de la table post
  let postsQuery = dbClient
    .from("posts")
    .select("*, author: profiles(*), likes(user_id), comments(*)", {
      count: "exact", //para saber el numero de posts que hay
    })
    .order("created_at", { ascending: false }) //lastest post will be the first
    .range((page - 1) * limit, page * limit - 1); //para la pagination
